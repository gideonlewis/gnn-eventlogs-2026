...
class TimeAwareGATConv(GATConv):
    ...
    def forward(self, x, edge_index, edge_attr=None, return_attention=False):
        ...
        # Lift to per-edge node features (avoid GATConv JIT-compiled propagate)
        row, col = edge_index          # row = source, col = target
        x_j = x[row]                   # [E, H, C] — source features
        x_i = x[col]                   # [E, H, C] — target features

        alpha = self.edge_attention(x_i, x_j, edge_attr)  # [E, H]
        self._alpha = alpha

        msgs = x_j * alpha.unsqueeze(-1)  # [E, H, C]

        # Aggregate (sum) messages to target nodes
        out = torch.zeros(N, H, C, dtype=x.dtype, device=x.device)
        if E > 0:
            col_idx = col.view(E, 1, 1).expand(E, H, C)
            out.scatter_add_(0, col_idx, msgs)
...
def get_top_significant_ranges(pairwise_table, num_top=10):
    ...
    # Dùng cột tạm để sắp xếp, tránh ghi đè cột string
    sig_comparisons['_p_val_sort'] = sig_comparisons['p-value'].astype(float)
    sig_comparisons['star_count'] = sig_comparisons['Significance'].str.len()
    sig_comparisons = sig_comparisons.sort_values(
        by=['star_count', '_p_val_sort', 'Cohen\'s d'],
        ascending=[False, True, False]
    )
    ...
    # p-value đã là string từ generate_pairwise_ttest_table, chỉ cần bỏ cột tạm
    formatted_comparisons = top_comparisons.drop(columns=['star_count', '_p_val_sort'])
